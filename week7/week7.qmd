...
**RESPONSE:** Using a single state, especially Texas, as a comparison likely doesn't have similar energy share or emissions.
...
**RESPONSE:** Close tracking means that the synthetic control would be a good match. If there isn't close tracking, we are unable to compare the synthetic control with with treated unit.
...
**RESPONSE:** We need to factor in the emissions because the synthetic control likely won't have the same CO2 emissions, to create a more accurate version of synthetic CA.
...
scm_data <- data %>%
  synthetic_control(
    outcome = co2_power_pc_tons, # Set outcome variable (per capita CO2)
    unit = state, # unit Identifying (state)
    time = year, # Time identifier
    i_unit = "CA", # Intervention (treatement) unit
    i_time = 2013, # Intervention (treatment) year
    # Create placebo tests for all states
    generate_placebos = TRUE) %>%
  
  # Defining which characteristics must match to be considered a synthetic CA
  generate_predictor(  
    time_window = 2000:2012, # Pre treat. window to compute predictors
    coal_share = mean(coal_share, na.rm = TRUE), # average share of electricity
    ...
    co2_pre_mean = mean(co2_power_pc_tons, na.rm = TRUE)) %>% # avg pre treatment outcome
  
  # Take the predictor summaries and figure out how much weight to give each donor state
  generate_weights(
    optimization_window = 2000:2012, # period over which donor weights are optimized
    margin_ipop = 0.02, # Smaller = more precise
    sigf_ipop = 7, # How many significant figures to use
    bound_ipop = 6) %>% 
  
  # Construct synthetic CA using optimized donor weights
  generate_control() 
```
...
**RESPONSE:** The observerd and synthetic track fairly well. This means the synthetic is credible and found optimal donor weights.

2.  What happens to the gap between the two lines after 2013? What does this suggest about the effect of cap-and-trade?

**RESPONSE:** The gap suggest the cap-and-trade policy was effective in reducing CO2 emissions.
...
**RESPONSE:** Pre-treatment, we see the differenc in the synthetic from the treated ocsillate around 0, but post treatment, the difference consistnely stays below 0.
...
**RESPONSE:** CA gap does stand out more than the rest. 
...
**RESPONSE:** CA is number 1, which means its post MSPE is much larger than pre MSPE. p value is calculated by position in ranking by the number of donors. (1/49) ~ 0.02
...
**RESPONSE:** Fairly well, with the exception of electricity prices

2.  How does the synthetic control compare to the simple donor pool average?

**RESPONSE:** Synthetic CA is much closer to CA than the donor sample is.
...
**RESPONSE:** Based on the results, we can say the cap and trade policy lowered 
...
**RESPONSE:** Our analysis had a good pre-treatment fit, wee excluded the treated states, there was favorable placebo comparison but there could be spillover.
...
**RESPONSE:** We would have seen compariable results to CA which would decrease robustness of our CA result. Our comparison would not be apples to apples which could shrink outcome to 0
...
**RESPONSE:** Including the pre treatment average as a predictor to account for differences across states' emissions.
