...
```{r}
rdd_lm <- ggplot(data_binned, aes(avg_distance, avg_deforest, color = as.factor(protected))) +
...
**RESPONSE**: The red line represents deforestation rate in non-protected forest and the blue represents deforestation rate in protected areas. The positive values are distance from the border within the protected area, whereas the negative values are the distance from the border outside the protected area. If there was no discontinuity, the data would suggest that protected areas do not decrease deforestation.
...
**RESPONSE**: Based on the results, on average we would expect protected area status to decrease deforestation by 10%. This matches in our plot above. The interaction coefficient represents the change of slope in protected areas vs in unprotected areas.
...
**RESPONSE**: In the local polynomial regression we account for weight of points that are closer to the border which influence our model accuracy. Only using -2 to 2 from the optimized bandwidth, which decreases the number of observations which creates a more conservative estimation. The local polynomial is also not linear where the OLS was.
...
**RESPONSE**: The local polynomial represents the trends of tehe points better, because it is able to follow the curves of the points.
333
...
**RESPONSE**: We see mostly similar trends as Neal 2024, however there appears to be more discontinuity in the simulated data than the actual study. 
