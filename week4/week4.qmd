...
*Response:* Reef exposure (exposed) and mangrove distance are very different between MPA and non-MPA.
...
*Response:* Reef exposure would effect fish biomass, because more exposed water would bring in more nutrients and upwelling, leading to trophic cascades increasing fish abundance.
...
*Response:* In this study `ATT`, refers to the average increase in fish biomass in MPAs vs. if they were not protected.
...
*Response:* All treated units were matched, and 37 unique control sites were matched.
...
reg_ecological <- lm(biomass_ecological~ treated_mpa,
                     data = matched_data, 
                     weights = weights) # account for controls being used multiple times
...
*Response:* On average, MPAs increased targeted fisheries biomass by ~108 kg/ha.

**Q8.** What is the biggest remaining threat to causal interpretation of regression coefficients after matching?

*Response:* The sample size is very small leading to a large SD and statistically insignificant p value. There is selection bias from unobserved variables.
...
*Response:* The `vpd` coefficient increases inthe fixed effect model, which suggests that plot differences were confounding the model.
...
*Response:* A pest outbreak or an extremely rainy year could impact the effects for one year but not the next
...
*Response:* Spatial variation of the plots such as soil type or exposure to streams could cause differences in plot effects.
