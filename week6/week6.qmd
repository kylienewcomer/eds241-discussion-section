...
```{r}
#options(noaakey = "McEXnDKUJRipPnvaaoXZWfqxseicLTDQ")
```
...
:::
53 observations, which is small.
...
```{r}
ggplot(data_weekly_joined, aes(pm25_mean, asthma_index)) +
  geom_point(alpha =0.6, color = "darkgreen", size = 2) +
  geom_smooth(method = "lm", se = TRUE, color = "cyan4") +
  theme_light() +
  labs(x = "Weekly PM 2.5 (AQI)",
       y = "Asthma Inhaler index",
...
```{r}
ggplot(data_weekly_joined, aes(precip_total, pm25_mean)) +
  geom_point(alpha =0.6, color = "darkgreen", size = 2) +
  geom_smooth(method = "lm", se = TRUE, color = "cyan4") +
  theme_light() +
  labs(x = "Weekly Precipitation (Inches)",
       y = "Weekly PM 2.5 (AQI)",
       title = "Precipitation vs PM 2.5")
...
**RESPONSE:** The coefficient increased and the pvalue decreased though it was still insignificant. This means that a seasonal confounder was heavily influencing the relationship. There is still bias with the small sample size and other potential influences from week to week patterns.
...
```{r}
first_stage <- lm(pm25_mean ~ precip_total + factor(month(week_start)), data = data_weekly_joined)
...
**RESPONSE:** The sign of the coefficient is negative, which suggests precipitation decreases PM 2.5, as we expect. I would consider precipitation a weak instrument
...
**RESPONSE:** Preciptation could cause people to stay inside, which could increase inhalation of dust or indoor air. 
...
**RESPONSE:** Increasing sample size could potentially increase precipitation.
...
**RESPONSE:** Wind speeds which could influence PM 2.5 and pollen or dust exposure increase asthma searches.
...
**RESPONSE:**
Exclusion restriction- instrumental variable (preciptation) must be correlated with treatment (pm2.5)

Relevance - 

Independence - 

Monotaniscicy - is there a scenario where precipitation decreases asthma index
