...
```
Without any treatment, we expect the number of waste piles to be ~78 on average. 
On average, any treatment will decrease the number of waste piles by ~25.
...
```
Intercept: On average, the model estimates our control group to have 74 waste piles
Coefficient: The model estimates that the treatment reduced waste burning by 34% in treated neighborhoods.
...
export_summs(m1_ols, m2_pois, m3_log,
               model.names = c("OLS", "Poisson", "Log(Outcome) OLS"),
               statistics = "none")
```
...
m2_change = (exp(-0.38)- 1) * 100 # Change in Poisson # -31.6
...
```{r}
m4_control <- lm(
  log(waste_piles) ~ treat + log(pre_count_avg),
  data = post_treat_subset
)

summ(m4_control, model.fit = TRUE)
```
Intercept: On average, when pre count avg is 0, the control areas would expect to have 2 trash piles.
Coefficient: On average, we expect a 24% reduction in waste piles, when controlling for pre treatment waste piles.
log(pre_count): a 1% increase in pre count average

Let's compare our two log OLS Regression Models. 

```{r}
export_summs(m3_log, m4_control,
             model.names = c("Log OLS regression", "Log OLS regression \n(with control)"),
             statistics = "none")
```
