...
# Create gglot
ggplot(weekly_la_phx, aes(week, mean_no2, color = city)) +
  geom_line(size = 1.2) +
  geom_point(size = 2.5) +
  ...
  geom_vline(xintercept = as.Date("2020-03-19"), 
             ...
             size = 1) +
  labs(x = "Date",
       y = "Average weekly max NO2 (ppb)",
       title = "Weekly NO2 Trends in 2020: Los Angeles vs. Phoenix",
       color = "City") +
  annotate("label", x = as.Date("2020-03-19"),
           y = 35,
           label = "CA Lockdown \nStarts",
           size = 3)
```
...
Violates the parallel trends assumption.
...
y_trt_post <- did_data_nm_tx %>% 
  ...
  summarize(mean(daily_max_1_hour_no2_concentration)) %>% 
  pull()
...
Yes, the fluctuations are fairly parallel with each other, but its harder to predicted because of the short tiime period with lots of fluctuation.
...
-3.516 with a p vlaue of 0.2210, which is not signicant. This means there is a 22% chance this result would have occured without the lockdown
...
El Paso county which is better because they are geographically close, but in different states. A dust storm that hits both counties would not ruin DiD because they both experience it. 
...
The DiD estimate would be smaller because we would see the same trends in both places, because the control group is following what the treated group would do. 