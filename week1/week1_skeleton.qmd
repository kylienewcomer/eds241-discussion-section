...
balance_summary %>% 
 ...
 tab_header(
  ...
  tab_source_note(
    source_note = "Note: Data fomr the Winter 2026 Class Survey"
  )
...
```{r}
tbl_2way <- lobsters %>% 
  # Calculate the total cpue for each year and region
...
tbl_3way %>% 
  gt(rowname_col = "year") %>% 
  tab_header(
    title = "European Lobster Catch by Treatemnet, Region, and Year",
...
table_w_plots <- lobsters %>% 
  ...
  summarize(total_cpue = sum(cpue, na.rm = TRUE),
            dist_cpue = list(cpue), # store all individual cpue values as a list each year)
            .groups = "drop"
  ) %>% 
  arrange(year) %>% 
  gt() %>% 
...
library(cowsay)
library(beepr)
say("All done making some beautiful tables! :) ", "endlesshorse"); beep(3)
